...
class ParseLiveListWidget<T extends ParseObject> extends StatefulWidget {
  ...
  static Widget defaultChildBuilder<T extends ParseObject>(
    ...
    if (snapshot.failed) {
      ...
      child = ListTile(
        title: Text(
          snapshot.loadedData?.get<String>(keyVarObjectId) ?? 'Missing Data!',
        ),
      );
    } else {
...
class _ParseLiveListWidgetState<T extends ParseObject>
  ...
  void initState() {
    ...
    ).then((ParseLiveList<T> liveList) {
      setState(() {
        _noData = liveList.size == 0;
        _liveList = liveList;
        liveList.stream.listen((ParseLiveListEvent<ParseObject> event) {
              ...
              animatedListState.removeItem(
                  ...
                  (BuildContext context, Animation<double> animation) =>
                      ParseLiveListElementWidget<T>(
                        key: ValueKey<String>(
                            event.object.get<String>(keyVarObjectId) ??
                                'removingItem'),
  ...
  Widget build(BuildContext context) {
    final ParseLiveList<T>? liveList = _liveList;
    if (liveList == null) {
  ...
  Widget buildAnimatedList(ParseLiveList<T> liveList) {
    return AnimatedList(
...
class _ParseLiveListElementWidgetState<T extends ParseObject>
  ...
  late ParseLiveListElementSnapshot<T> _snapshot;
  StreamSubscription<T>? _streamSubscription;

  @override
  void initState() {
    _snapshot = ParseLiveListElementSnapshot<T>(
